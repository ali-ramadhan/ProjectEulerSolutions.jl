...
We shall make no distinction between "Just Visiting" and being sent to JAIL, and with CH the
condition is that a player is sent to the nearest utility if they draw the "utility" card
and are on CH1, they will go to U1; if they are on CH2, they will go to U2; if they are on
CH3, they will go to U1 (wrapping around).

At the beginning of the game, the CC and CH cards are shuffled. When a player lands on CC or
CH they take a card from the top of the respective pile and, after following the
...
So, the most popular squares, in order, are JAIL (6.24%) = Square 10, E3 (3.18%) = Square
24, and GO (3.09%) = Square 00. So these give the six-digit modal string 102400.
...
This problem requires Monte Carlo simulation to estimate the stationary probabilities of a
Markov chain representing the Monopoly game. We simulate millions of dice rolls, tracking
which squares are landed on most frequently.

The simulation handles all the complex rules: G2J squares, CC/CH cards, consecutive doubles,
and secondary movements when cards redirect players to other special squares.

## Complexity analysis

Time complexity: O(n)
- Where n is the number of dice rolls simulated
- Each roll involves constant-time operations: dice rolling, position updates, card draws

Space complexity: O(1)
- Fixed-size arrays for visit counts (40 squares)
- Constant space for game state (position, doubles count, etc.)

## Key insights

The problem showcases how complex rules in board games create non-uniform probability
distributions. JAIL becomes the most visited square due to multiple paths leading there: G2J
square, CC/CH cards, and the three-consecutive-doubles rule.
"""
module Problem084
...
function solve()
    ...
    # Find the three most visited squares and their frequencies
    sorted_indices = sortperm(visit_counts; rev = true)
    top_three = [(sorted_indices[i] - 1, visit_counts[sorted_indices[i]]) for i in 1:3]

    @info "Monopoly probability analysis with 4-sided dice:"
    @info "Most visited squares: " *
          "$(BOARD_SQUARES[top_three[1][1]+1]) " *
          "($(round(100*top_three[1][2]/num_rolls, digits=2))%), " *
          "$(BOARD_SQUARES[top_three[2][1]+1]) " *
          "($(round(100*top_three[2][2]/num_rolls, digits=2))%), " *
          "$(BOARD_SQUARES[top_three[3][1]+1]) " *
          "($(round(100*top_three[3][2]/num_rolls, digits=2))%)"
