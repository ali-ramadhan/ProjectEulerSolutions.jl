...
is known that the greatest number that cannot be expressed as the sum of two abundant
...
1. Find all abundant numbers up to the limit (28123)
2. Create a boolean array to mark all numbers that can be expressed as the sum of two
   abundant numbers
3. For each pair of abundant numbers, mark their sum if it's within the limit
4. Sum all numbers from 1 to the limit that were not marked

This approach uses the mathematical fact that all integers greater than 28123 can be written
as the sum of two abundant numbers.

## Complexity analysis

Time complexity: O(a² + n)
- Finding all abundant numbers takes O(n × √d) where n is the limit and d is the divisor
  range
- Marking sums of abundant numbers takes O(a²) where a is the number of abundant numbers
  (≈ 6000)
- Summing unmarked numbers takes O(n)

Space complexity: O(n)
- We store a boolean array of size n to track which numbers can be expressed as abundant
  sums
"""
module Problem023
