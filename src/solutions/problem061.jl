...
Calculate the nth s-gonal (figurate) number.
For s=3 (triangle), s=4 (square), s=5 (pentagonal), etc.
"""
...
Generate figurate numbers of type s (3 for triangle, 4 for square, etc.)
that are within the range [min_val, max_val].
"""
...
Recursively find a cyclical set of figurate numbers.
Returns the path if a cyclical set is found, nothing otherwise.
"""
...
Find the ordered set of six cyclic 4-digit numbers where each number is a different
figurate type (triangle, square, pentagonal, hexagonal, heptagonal, and octagonal).
