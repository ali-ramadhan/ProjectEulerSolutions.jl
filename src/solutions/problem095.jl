...
We use Floyd's cycle detection algorithm to find amicable chains efficiently:

1. For each unprocessed number, follow the chain of proper divisor sums
2. Use tortoise-and-hare method to detect cycles without storing the entire chain
3. Once a cycle is found, measure its length and find the minimum element
4. Track the longest chain found and return its minimum element

## Complexity analysis

Time complexity: O(n√m) where n ≤ 10^6 and m is the maximum number in chains
- For each number up to n, compute proper divisors: O(√m)
- Floyd's cycle detection: O(chain_length) per chain

Space complexity: O(n)
- Set to track processed numbers to avoid recomputation
"""
module Problem095
...
function solve()
    length, smallest = find_longest_amicable_chain(1_000_000)
    @info "Longest amicable chain has length $(length) with smallest member $(smallest)"
    return smallest
end
